...
#calls also XPSConstrain() to set links
           ...
           if (exists("TKwinGraph", envir=.GlobalEnv)){ #XPSMoveCompoonent GUI is active to beused in combination with XPSConstraintsGUI
               assign("Object", FName[[SpectIndx]], envir=.GlobalEnv)
               tkrreplot(Graph)
           } else {
...
#handler to control consistency of parameter selection
      ...
      HndlrSetLinks <- function(){
                         ...
                         LinkGroup1 <- ggroup(horizontal=FALSE, spacing=8, container=LinkFCwin)
                         LinkFrame1 <- gframe(text="SELECT LINKS", horizontal=FALSE, spacing=3, container=LinkGroup1)
                         ...
                         LinkGroup2 <- ggroup(horizontal=FALSE, spacing=3, container=LinkFrame1)
                         Linklayout <- glayout(homogeneous=FALSE, spacing=2, container=LinkFrame1)
                         ...
                         for (jj in 1:NfitC){  #first row of the Table is dedicated to FitComponent names == FitComp1
                             Linklayout[1, jj+1] <- glabel(FitComp1[jj], spacing=2, container=Linklayout)
                         }
                         for (ii in 1:NfitP){  #first column of the Table is dedicated to FitParameter names == ParamLbl
                             Linklayout[ii+1,1] <- glabel(ParamLbl[ii], spacing=2, container=Linklayout)
                             T3LinkParam[[ii]] <<- list()
                         }
                         ...
                         for (jj in 1:NfitC){  #run on Fit Components
                             ...
                             for (ii in 1:NfitP){  #For the FitComp[jj] build the correspondent Param Column
                                 if (PP[ii] > 0){
                                    ...
                                    Linklayout[ii+1, jj+1] <- T3LinkParam[[ii]][[jj]] <<- gcheckbox("     ", selected=-1, spacing=2, handler=function(h, ...){
                         ...
                         LinkFrame2 <<- gframe(text="REFERENCE COMPONENT", horizontal=FALSE, spacing=3, container=LinkGroup1)
                         ...
                         linkExpression <<- ""
                         LinkFrame3 <- gframe(text="LINK OPERATIONS", horizontal=FALSE, spacing=2, container=LinkGroup1)
                         glabel("Example1: link mu(C2) to mu(C1) and set Link Expression +1.3", spacing=1, container=LinkFrame3)
                         glabel("        Position of C2 will be mu(C1) + 1.3eV", spacing=1, container=LinkFrame3)
                         glabel("Example2: link h(C4) to h(C3) and set LinkExpression *0.5", spacing=1, container=LinkFrame3)
                         glabel("        Intensity of C4 will be h(C3) * 0.5", spacing=1, container=LinkFrame3)
