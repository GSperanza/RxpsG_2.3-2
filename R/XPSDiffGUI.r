...
XPSDiff <- function(){
   ...
   SaveFName <- function(saveMode){
      ...
      Nchr <- nchar(Symbol)
      chrPos <- gregexpr(pattern ="D.",Symbol)  #the result of gregexpr is a list containing the character position of D.
      chrPos <- chrPos[[1]][1]
...
#save data in OVERWRITE mode
      if (chrPos > 0 && saveMode=="Overwrite") { #"D." is present in Symbol: overwrite previously differentiated data 
                                                 #a differentiation of previously differentiated data was made
         FName[[SpectIndx]]@.Data <<- Differentiated  #change the original diff. data with the new differented data
         DDeg <- DDeg+as.integer(substr(Symbol,chrPos+2,chrPos+2))  #sum of the previous differentiation degree with the actual differentiation degree
         substr(Symbol,chrPos+2,chrPos+2) <- as.character(DDeg)     #update the differentiation degree with the actual one
         DDeg <- as.integer(DDeg)
         Info <- FName[[SpectIndx]]@Info
         nI <- length(Info)
      }
      if (chrPos < 0 && saveMode=="Overwrite") { #"D." NOT present, overwrite non-differentited data
         idx <- length(FName)+1                 #number of corelines +1
         FName[[idx]] <<- FName[[SpectIndx]]    #Add the new Coreline.
...
# --- Line 1 ---
    ...
    D1Frame3 <- gframe(" Update Coreline List ", spacing=5, container=D1group2)
