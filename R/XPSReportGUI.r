   ...
   MakeQuantReport <- function(){
      ...
      for(ii in SelectedCL1){  #SelectedCL1 contains the analyzed CoreLines excluding VB Auger spectra
          ...
          if(N_Comp == 0){  #Contribution of NON-fitted BKG-subtracted Core-Lines
             ...
             TotArea <- TotArea + sum(XPSSample[[ii]]@RegionToFit$y-XPSSample[[ii]]@Baseline$y)/RSF
          } else {
      ...
      for(ii in SelectedCL1){
          ...
          RSF <- XPSSample[[ii]]@RSF
          AreaCoreLine <- sum(NormAreaComp[[ii]])  #sum(XPSSample[[ii]]@RegionToFit$y-XPSSample[[ii]]@Baseline$y)
          Area <- sprintf("%1.2f", AreaCoreLine)  #round to 2 decimals and transform to string
          Conc <- sprintf("%1.2f",100*AreaCoreLine/TotArea)
...
#---Widget
   ...
   txtWin <- gwindow(title="XPS Sample Report", parent=c(10, 50), visible=FALSE)
