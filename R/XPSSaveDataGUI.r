...
XPSSaveData <- function() {
     ...
     SaveSingle <- function(){
          ...
          if (PathName != getwd()){  #original folder different from the current wirking directory
             txt="Warning: current directory not appropriate. Please check the Destination Folder"
             gmessage(msg=txt, title="SET DESTINATION FOLDER", icon="warning")
             ChDir()
          }
     ...
     SaveAll <- function(){
          ...
          for(jj in 1:LL){
              ...
              idx <- gregexpr(pattStr, xxx, fixed=FALSE)
              idx <- unlist(idx)
